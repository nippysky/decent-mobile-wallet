...
export default function SendTokensConfirm() {
  const { amount, receiverAddress, name, symbol, icon, contractAddress } =
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await sendTransaction({
        sender,
        contractAddress: contractAddress as string, // ✅ Include contractAddress
      ...
      if (response.result?.status === 1) {
        ...
        router.replace("/(SCREENS)/wallet"); // ✅ Navigate back to wallet screen
      } else {
